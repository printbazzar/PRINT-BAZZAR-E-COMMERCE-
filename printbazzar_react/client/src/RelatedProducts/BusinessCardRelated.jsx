...
export function BusinessCardRelated() {
      ...
      <div className=" mx-auto px-3">
        ...
        <div className="max-w-full grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 2xl:grid-cols-6  gap-6">
          {randomProducts.map((product, index) => (
            <Link
              to={`/${product.title.toLowerCase().replace(/ /g, "")}Details`} // Dynamic link based on product name
              key={index}
            ...
            >
              ...
              />
              <h3 className="text-md md:text-lg font-medium pt-3">{product.title}</h3>
            </Link>
          ))}
        </div>
      </div>
