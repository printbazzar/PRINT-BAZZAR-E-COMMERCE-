import {
    Breadcrumb,
    Button,
    Checkbox,
    Label,
    Modal,
    Select,
    } from "flowbite-react";
    import Image1 from "../assets/images/business_card/die_cutting.jpg";
    import Thumbnail1 from "../assets/images/business_card/die_cutting.jpg";
    import Thumbnail2 from "../assets/images/business_card/die_cutting_mtrl.jpg";
    import Thumbnail3 from "../assets/images/business_card/laminate_mtrl_2.png";
    import dc1 from '../assets/images/business_card/dc1.jpg'
    import dc2 from '../assets/images/business_card/dc2.jpg'
    import dc3 from '../assets/images/business_card/dc3.jpg'
    import dc4 from '../assets/images/business_card/dc4.jpg'
    import dc5 from '../assets/images/business_card/dc5.jpg'
    import dc6 from '../assets/images/business_card/dc6.jpg'
    import dc7 from '../assets/images/business_card/dc7.jpg'
    import dc8 from '../assets/images/business_card/dc8.jpg'
    import dc9 from '../assets/images/business_card/dc9.jpg'
    import dc10 from '../assets/images/business_card/dc10.jpg'
    import dc11 from '../assets/images/business_card/dc11.jpg'
    import dc12 from '../assets/images/business_card/dc12.jpg'
    import dc13 from '../assets/images/business_card/dc13.jpg'
    import dc14 from '../assets/images/business_card/dc14.jpg'
    import { useState, useEffect } from "react";
    import { Link } from "react-router-dom";
    import GuideDesign from "../Components/GuideDesign";
    import { HiClock, HiHome } from "react-icons/hi";
import { BusinessCardRelated } from "../RelatedProducts/BusinessCardRelated";
    
    export function DieCuttingCardDetails() {
    const [mainImage, setMainImage] = useState(Image1);
    const [printingType, setPrintingType] = useState("Double Side");
    const [lamination, setLamination] = useState("Matte");
    const [dieShape, setDieShape] = useState("SHAPE-1");
    const [quantity, setQuantity] = useState("1000");
    const [design, setDesign] = useState("No Thank You");
    const [totalPrice, setTotalPrice] = useState(0);
    const [agree, setAgree] = useState(false);
    const [openModal, setOpenModal] = useState(false);
    
    const handleCheckboxChange = () => {
      setAgree(!agree);
    };
    
    useEffect(() => {
      calculatePrice();
    }, [printingType, design, quantity]);
    
    const handleThumbnailClick = (image) => {
      setMainImage(image);
    };
    
    const calculatePrice = () => {
      let price = 0;
    
      if (printingType === "Double Side") {
        if (design === "Yes Please") {
          if (quantity === "1000") price = 1500 + 400;
        } else if (design === "No Thank You") {
          if (quantity === "1000") price = 1500;
        }
      }
    
      setTotalPrice(price);
    };
    
    const handleSubmit = () => {
      const message = `
        *Die Cutting Business Card (#PB0013)*:
        - Lamination: ${lamination}
        - Die Shapes: ${dieShape}
        - Printing Type: ${printingType}
        - Quantity: ${quantity}
        - Design: ${design}
        - Price: ₹${totalPrice} incl (GST) + Shipping charges\n
        *7 Days Delivery (From Ordered Date)*
    
      `;
    
      const encodedMessage = encodeURIComponent(message);
      const targetNumber = (typeof window !== "undefined" && window.__BUSINESS_WHATSAPP__) ? window.__BUSINESS_WHATSAPP__ : "919629098565";
      const whatsappUrl = `https://wa.me/${targetNumber}?text=${encodedMessage}`;
    
      window.open(whatsappUrl, "_blank");
    };
    
    return (
      <div className="mx-auto px-4 py-8 max-w-full">
        <div className="px-4 py-2">
          <Breadcrumb className="text-base lg:text-lg xl:text-xl 2xl:text-2xl">
            <Breadcrumb.Item icon={HiHome}>
              <Link to="/" className="hover:underline">
                Home
              </Link>
            </Breadcrumb.Item>
            <Breadcrumb.Item>
              <Link to="/shop" className="hover:underline">
                Shop
              </Link>
            </Breadcrumb.Item>
            <Breadcrumb.Item>
              <Link to="/category/Business%20Cards" className="hover:underline">
                Business Cards
              </Link>
            </Breadcrumb.Item>
            {/* Current Path - Truncate and Wrap */}
          </Breadcrumb>
        </div>
        <div className="mt-5 flex flex-col justify-center md:flex-row">
          {/* Left Side: Main Image and Thumbnails */}
          <div className="w-full md:w-1/3 2xl:w-1/3 lg:sticky lg:top-0">
            {
            dieShape == 'SHAPE-1' ?
          <img
          src={dc1}
          alt="dc-1"
          className="w-full h-auto object-cover rounded-lg"
        />
        :
        dieShape == 'SHAPE-2' ?
          <img
          src={dc2}
          alt="dc-2"
          className="w-full h-auto object-cover rounded-lg"
        />
        :
        dieShape == 'SHAPE-3' ?
          <img
          src={dc3}
          alt="dc-3"
          className="w-full h-auto object-cover rounded-lg"
        />
        :
        dieShape == 'SHAPE-4' ?
          <img
          src={dc4}
          alt="dc-4"
          className="w-full h-auto object-cover rounded-lg"
        />
        :
        dieShape == 'SHAPE-5' ?
          <img
          src={dc5}
          alt="dc-5"
          className="w-full h-auto object-cover rounded-lg"
        />
        :
        dieShape == 'SHAPE-6' ?
          <img
          src={dc6}
          alt="dc-6"
          className="w-full h-auto object-cover rounded-lg"
        />
        :
        dieShape == 'SHAPE-7' ?
          <img
          src={dc7}
          alt="dc-7"
          className="w-full h-auto object-cover rounded-lg"
        />
        :
        dieShape == 'SHAPE-8' ?
          <img
          src={dc8}
          alt="dc-8"
          className="w-full h-auto object-cover rounded-lg"
        />
        :
        dieShape == 'SHAPE-9' ?
          <img
          src={dc9}
          alt="dc-9"
          className="w-full h-auto object-cover rounded-lg"
        />
        :
        dieShape == 'SHAPE-10' ?
          <img
          src={dc10}
          alt="dc-10"
          className="w-full h-auto object-cover rounded-lg"
        />
        :
        dieShape == 'SHAPE-11' ?
          <img
          src={dc11}
          alt="dc-11"
          className="w-full h-auto object-cover rounded-lg"
        />
        :
        dieShape == 'SHAPE-12' ?
          <img
          src={dc12}
          alt="dc-12"
          className="w-full h-auto object-cover rounded-lg"
        />
        :
        dieShape == 'SHAPE-13' ?
          <img
          src={dc13}
          alt="dc-13"
          className="w-full h-auto object-cover rounded-lg"
        />
        :
        dieShape == 'SHAPE-14' ?
          <img
          src={dc14}
          alt="dc-14"
          className="w-full h-auto object-cover rounded-lg"
        />
        :
         <img
        src={mainImage}
        alt="Main Image"
        className="w-full h-auto object-cover rounded-lg"
        />
          }
            <div className="mt-4 flex space-x-4 justify-start">
              <img
                src={Thumbnail1}
                alt="Thumbnail 1"
                className="w-16 h-16 xl:w-32 xl:h-32 object-cover rounded-lg cursor-pointer"
                onClick={() => handleThumbnailClick(Thumbnail1)}
              />
              <img
                src={Thumbnail2}
                alt="Thumbnail 2"
                className="w-16 h-16 xl:w-32 xl:h-32 object-cover rounded-lg cursor-pointer"
                onClick={() => handleThumbnailClick(Thumbnail2)}
              />
              <img
                src={Thumbnail3}
                alt="Thumbnail 3"
                className="w-16 h-16 xl:w-32 xl:h-32 object-cover rounded-lg cursor-pointer"
                onClick={() => handleThumbnailClick(Thumbnail3)}
              />
            </div>
          </div>
    
          {/* Right Side: Product Info and Form */}
          <div className="md:ml-6 mt-4 md:mt-0 w-full md:w-1/2 2xl:w-1/2">
            <h3 className="font-medium text-3xl">Die Cutting Card</h3>
            <label className="block font-medium text-sm text-gray-700 mt-2">
                Select the suitable shape for your card out of 14 Model 
              </label>
            {/* Form Fields */}
            
            <div className="mt-4">
            <label className="block font-medium text-sm text-gray-700">
              Lamination
            </label>
            <Select
              value={lamination}
              onChange={(e) => setLamination(e.target.value)}
            >
              <option>Matte</option>
            </Select>
          </div>

            <div className="mt-4">
              <label className="block font-medium text-sm text-gray-700">
                Die Shapes
              </label>
              <Select
                value={dieShape}
                onChange={(e) => setDieShape(e.target.value)}
              >
                <option>SHAPE-1</option>
                <option>SHAPE-2</option>
                <option>SHAPE-3</option>
                <option>SHAPE-4</option>
                <option>SHAPE-5</option>
                <option>SHAPE-6</option> 
                <option>SHAPE-7</option>
                <option>SHAPE-8</option>
                <option>SHAPE-9</option>
                <option>SHAPE-10</option>
                <option>SHAPE-11</option>
                <option>SHAPE-12</option>
                <option>SHAPE-13</option>
                <option>SHAPE-14</option>
                <option>Custom Shapes (Extra die charges)</option>
              </Select>
              
            </div>

            <div className="mt-4">
              <label className="block font-medium text-sm text-gray-700">
                Printing Location
              </label>
              <Select
                value={printingType}
                onChange={(e) => setPrintingType(e.target.value)}
              >
                <option>Double Side</option>
              </Select>
            </div>
    
    
            <div className="mt-4">
              <label className="block font-medium text-sm text-gray-700">
                Quantity
              </label>
              <Select
                value={quantity}
                onChange={(e) => setQuantity(e.target.value)}
              >
                <option>1000</option>
              </Select>
            </div>
    
            <div className="mt-4">
              <label className="block font-medium text-sm text-gray-700">
                Let Us Design For You
              </label>
              <Select value={design} onChange={(e) => setDesign(e.target.value)}>
                <option>Yes Please</option>
                <option>No Thank You</option>
              </Select>
            </div>
    
            <div className="mt-4">
              <label className="block font-medium text-sm text-gray-700">
                Price
              </label>
              <h3 className="font-medium  text-2xl sm:text-3xl text-red-700">
                ₹{totalPrice}{" "}
                <span className="text-gray-500 text-sm sm:text-base">(incl GST)</span> +{" "}
                <span className="text-gray-500 text-sm sm:text-base">Shipping charges</span>
              </h3>
            </div>
            <div className="mt-4">
              <h3 className="font-medium text-3xl flex gap-2 text-red-700">
                <span className="text-sm sm:text-base flex items-center gap-2">
                  <HiClock className="text-sm sm:text-xl" />
                  10 Days Delivery
                </span>
                <span className="text-gray-500 text-sm sm:text-base">
                  (From Ordered Date)
                </span>
              </h3>
            </div>
            <div className="mt-4 flex items-center gap-2">
              <Checkbox id="accept" onChange={handleCheckboxChange} />
              <Label htmlFor="accept" className="flex text-sm sm:text-md">
                I agree with the&nbsp;
                <a
                  onClick={() => setOpenModal(true)}
                  className="text-red-600 hover:underline cursor-pointer"
                >
                  terms and conditions
                </a>
              </Label>
            </div>
    
            <div className="mt-4">
              <Button
                color="dark"
                onClick={handleSubmit}
                disabled={!agree}
                className="rounded-lg w-full"
              >
                Buy Now
              </Button>
            </div>
            {design === "No Thank You" && (
              <Button
                gradientDuoTone="purpleToBlue"
                className="mt-6 w-full hover:opacity-90"
                as={Link}
                to="https://www.canva.com/search?q=business%20cards"
                target="_blank"
              >
                Create your Own Design
              </Button>
            )}
          </div>
    
          <Modal show={openModal} onClose={() => setOpenModal(false)}>
            <Modal.Header>Terms & Conditions</Modal.Header>
            <Modal.Body>
              <div className="space-y-6">
                <p className="text-base leading-relaxed text-gray-700 dark:text-gray-400">
                  Please find the design proof for your order attached. Kindly
                  review it carefully and confirm that everything is correct,
                  including:
                </p>
                <ul className="text-base leading-relaxed text-gray-500 list-disc">
                  <li>Text (spelling, grammar, and alignment)</li>
                  <li>Colors, images, and design layout</li>
                  <li>Dimensions and bleed/crop marks (if applicable)</li>
                  <li>
                    If there are any changes needed, please let us know as soon as
                    possible. Once we receive your approval, we’ll proceed with
                    printing.
                  </li>
                </ul>
                <p className="text-base leading-relaxed text-gray-700 dark:text-gray-400">
                  ⚠ Please note: By confirming the proof, you acknowledge that the
                  design is final, and any errors after printing will be your
                  responsibility.
                </p>
              </div>
            </Modal.Body>
          </Modal>
        </div>
    
        <div>
          <GuideDesign />
        </div>

        <div className="mt-5 max-w-4xl 2xl:max-w-full xl:mt-20  flex flex-col ">
        <h3 className="text-dark text-2xl lg:text-3xl 2xl:text-4xl font-medium">
          Information
        </h3>
        <div className="px-5 mt-5">
          <ul className="text-base leading-relaxed text-gray-500 list-disc">
            <li>
              <span className="text-black font-medium">Artboard:</span>{" "}
              (300 GSM)
            </li>
            <li>
              <span className="text-black font-medium">Multicolor Printing Front & Back</span>{" "}
            </li>
            <li>
            <span className="text-black font-medium">Die Cut -  (Refer Product Catalogue)</span>
            </li>
            <li>
              <span className="text-black font-medium">Front & Back Matte Lamination</span>
            </li>
            <li>
              <span className="text-black font-medium">90mm x 53 mm Upload File size</span> 
            </li>
            <li>
              <span className="text-black font-medium">10 Days delivery time</span>
            </li>
          </ul>
        </div>
      </div>
    
        <div>
          <BusinessCardRelated />
        </div>
      </div>
    );
    }
    