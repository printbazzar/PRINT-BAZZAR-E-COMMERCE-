...
import Image1 from "../assets/images/business_essen/bulk billbook.jpg";
import Thumbnail1 from "../assets/images/business_essen/bulk billbook.jpg";
import Thumbnail2 from "../assets/images/business_essen/bulk billbook_mtrl.jpg";
import Thumbnail3 from "../assets/images/business_essen/bulk billbook_mtrl2.jpg";
...
export function BulkBillBookDetails() {
  const [mainImage, setMainImage] = useState(Image1);
  const [printing, setPrinting] = useState("Multi Color");
  const [padType, setPadtype] = useState("Original + 1 Duplicate");
  const [duplicateColorMulti, setduplicateColorMulti] = useState("Black & White");
  ...
  useEffect(() => {
    calculatePrice();
  }, [quantity, size, padType, design, duplicateColorMulti]);
       ...
       if (size.trim() === "A4") {
        if (padType === "Original + 1 Duplicate") {
          if (design === "Yes Please") {
            if (quantity === "10") price = 4000 + 200;
          } else if (design === "No Thank You") {
            if (quantity === "10") price = 4000;
          }
        }
      }
      else if (size.trim() === "A5") {
        if (padType === "Original + 1 Duplicate") {
          if (design === "Yes Please") {
            if (quantity === "20") price = 4100 + 200;
          } else if (design === "No Thank You") {
            if (quantity === "20") price = 4100;
          }
        }
      }
  ...
  const handleSubmit = () => {
    if(quantity === 'Select Quantity'){
        ...
        const message = `
            ...
            *6-7 Days Delivery (From Ordered Date)*
          `;
      ...
      <div className="mt-5 flex flex-col justify-center md:flex-row">
        ...
        <div className="md:ml-6 mt-4 md:mt-0 w-full md:w-1/2 2xl:w-1/2">
          <h3 className="font-medium text-2xl sm:text-3xl flex items-center gap-5">
            Bulk Bill Book (Multi color)<br /> <small className="text-base">(#PB0019)</small>
          </h3>
          ...
          <div className="mt-4">
            ...
            <Select value={size} onChange={(e) => {setSize(e.target.value); setQuantity("");}}>
              <option>A4</option>
              <option>A5</option>
            </Select>
          </div>
          ...
          <div className="mt-4">
            <h3 className="font-medium text-3xl flex gap-2 text-red-700">
              <span className="text-sm sm:text-base flex items-center gap-2">
                <HiClock className="text-sm sm:text-xl" />6-7 Days Delivery
              </span>
              <span className="text-gray-500 text-sm sm:text-base">
                (Order Before 12PM)
              </span>
